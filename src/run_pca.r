...
using("readxl", "ggplot2", "corrplot", "reshape", "pcaMethods")
...
# auth_pca <- prcomp(auth_vars, scale = TRUE)
auth_pca <- pca(auth_vars, method='ppca', nPcs=17, seed=123)
...
# loadings <- auth_pca$rotation[, 1:5]
loadings <- auth_pca@loadings[, 1:5]

# scores <- auth_pca$x[, 1:5]
scores <- auth_pca@scores[, 1:5]
